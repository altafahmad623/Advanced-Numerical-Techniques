...
// \frac{ \partial u}{\partial t}  = \nu [ \frac{ \partial^2 u}{\partial x^2} +  \frac{ \partial^2 u}{\partial y^2}] \; , -1 < x, y  < 1;
// \nu = 1,  u(x,y, 0 ) = cos (pi * x /2)*cos (pi * y /2) \And u(t, +- 1 , y) = 0 , u ( t, x, +- 1 ) = 0
// using ALternating direction implicit scheme
double init(double x, double y)
{
    double k = (cos((x * PI) / 2)) * (cos((y * PI) / 2));
    return k;
}

int main()
{
    double h = 0.5; // delta x
    double dt = 0.1, dx = h, dy = 0.5;
    int timesteps = 4;
    double u2 = 0.1, alpha = 0.01;
    //double dt = r * (h * h) / (mu);
    double x_0 = -1.0, y_0 = -1;
