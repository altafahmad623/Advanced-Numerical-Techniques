...
//change the functions B,C, and D which are in the equation: 
//y''  + B(x)y = C(x), they can be functions of x or constants.
...
{
    double aii = (6.0/(h*h) ) + B(x-h);
    return aii;
}
double b_i(double x, double h)
{
    double aii = -(12.0/(h*h) ) + 4*B(x);
    return aii;
}
...
double d_i(double x, double h)
{
    double aii = C(x-h) + 4*C(x) + C(x+ h);
    return aii;
}
int main()
{
